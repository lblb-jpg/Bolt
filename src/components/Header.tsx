...
export const Header: React.FC<HeaderProps> = ({ onOpenNotifications, notificationsOpen, profileName, onLogout }) => {
  return (
    <header className="glass-header fixed inset-x-0 top-0 z-50 isolate w-full pt-[env(safe-area-inset-top)]">
      <div className="mx-auto flex h-16 max-w-5xl items-center justify-between gap-1.5 px-2.5 min-[380px]:gap-2 min-[380px]:px-3.5 sm:h-[72px] sm:gap-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-2 min-[380px]:gap-2.5 sm:gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 hidden truncate text-[7px] font-medium uppercase tracking-[0.13em] text-slate-500 min-[360px]:block min-[430px]:text-[8px] min-[430px]:tracking-[0.16em] sm:text-[9px]">
              Espace {profileName}
            </p>
          </div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          ...
          >
            ...
            <span className="hidden min-[420px]:inline">{supabaseConfigured ? "En ligne" : "Local"}</span>
          </div>
